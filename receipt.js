var vision = require('@google-cloud/vision')({
    projectId: 'receipt-scanner-196000',
    ...
    vision.detectText(storage.bucket(bucket).file(img), {
      ...
      for(var j in sentences){
        ...
        for (i = 0; i < sr.length; i++) {
          console.log(sr[i] + " compare thiiiis word");
          if(sr[i].trim().toLowerCase() == "total"){
            possibleTotal.push(sr.slice(i+1));
          }
        }
        ...
        for (var iBack= sr.length; iBack--; iBack <=0 ) {
          ...
          // console.log(sr[0].trim().toLowerCase() + " compare thiiiis word");

          // if (sr.includes('total')) {possibleTotal.push(sr)}
      ...
      if (possibleTotal.length == 0) {
        possibleTotal.push("no possibleTotal found",result)

      }
      callback(possibleTotal);
